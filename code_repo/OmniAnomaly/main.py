    ...
    with tf.variable_scope('model') as model_vs:
        ...
        with tf.Session().as_default():
            ...
            if x_test is not None:
                ...
                test_score, test_z, pred_speed = predictor.get_score(x_test, False)
                ...
                if y_test is not None and len(y_test) >= len(test_score):
                    ...
                    specificity = t[4] / (t[4] + t[5])
                    sensitivity = t[3] / (t[3] + t[6])
...
if __name__ == '__main__':
    ...
    config.save_dir = os.path.join(folder,'model')

    config.result_dir = os.path.join(folder,'result')  # Where to save the result file
