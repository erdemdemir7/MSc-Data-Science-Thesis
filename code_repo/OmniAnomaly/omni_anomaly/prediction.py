...
class Predictor(VarScopeObject):
    ...
    def _get_score_without_y(self):
        if self._score_without_y is None:
            with reopen_variable_scope(self.variable_scope), \
                ...
                self._embed, self._score_without_y, self._q_net_z = self.model.get_score(
                    x=self._input_x,
                    n_z=self._n_z,
                    last_point_only=self._last_point_only
                )
                # print ('\t_get_score_without_y ',type(self._q_net_z))
        return self._embed, self._score_without_y, self._q_net_z
    ...
    def get_score(self, values, is_train):
        """
        ...
        with tf.name_scope('Predictor.get_score'):
            ...
            for b_x, in sliding_window.get_iterator([values]):
                ...
                embedding, b_r, q_net_z = sess.run(self._get_score_without_y(),
                                        feed_dict=feed_dict)

                # embedding_dict.update({counter: embedding[0]})
                # counter += 1
            ...
            # if is_train:
            #     which_embedding = '_train_'
            #     counter = 0
            # else:
            #     which_embedding = '_test_'
            #     folder = 'OmniAnomaly_' + self.model.config.dataset + '_' + str(self.model.config.window_length)
            #     prefix = str(self.model.window_length)+which_embedding+'embeddings.pkl'
            #     makedirs(folder, exist_ok=True)

            #     try:
            #         file = open(os.path.join(folder, prefix), "wb")
