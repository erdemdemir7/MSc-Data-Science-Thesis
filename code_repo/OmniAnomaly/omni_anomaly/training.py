            ...
            for epoch in loop.iter_epochs():
                ...
                for step, (batch_x,) in loop.iter_steps(train_iterator):
                    ...
                    if step % self._valid_step_freq == 0:
                        ...
                        #add train loss
                        train_loss_values = np.append(train_loss_values, loss)
                        ...
                        with loop.timeit('valid_time'), \
                            ...
                            for (b_v_x,) in v_it:
                                ...
                                # #add valid loss
                                # valid_loss_values = np.append(valid_loss_values, loss)
                ...
                if self._lr_anneal_epochs and \
                    ...
                    loop.println('Learning rate decreased to {}'.format(lr),
                                 with_tag=True)
            folder = 'OmniAnomaly_' + self.config.dataset + '_' + str(self.config.window_length)
            prefix = 'OmniAnomaly_' + self.config.dataset + '_' + str(self.config.window_length) + '_'
            makedirs(folder, exist_ok=True)

            try:
                f = open(os.path.join(folder, prefix + 'train_losses.npy'), "wb")
                np.save(f, train_loss_values)
                f.close()
            except (KeyError, FileNotFoundError):
                print('')
            
            # try:
            #     f = open(os.path.join(folder, prefix + 'valid_losses.npy'), "wb")
            #     np.save(f, valid_loss_values)
            #     f.close()
            # except (KeyError, FileNotFoundError):
            #     print('')

            # np.save('train_losses.npy', train_loss_values)
            # np.save('valid_losses.npy', valid_loss_values)
